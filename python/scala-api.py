...
for curdir, subdirs, files in walk(startdir):
    ...
    for f in files:
        ...
        if kind[0] == 'trait':
            packagefile.write(cmdtrt.format(name, package, inherits) + "\n")
        elif kind[0] == 'class':
            packagefile.write(cmdcls.format(name, package, inherits) + "\n")
        elif kind[0] == 'object':
            packagefile.write(cmdobj.format(name, package, inherits) + "\n")
    packagefile.close
